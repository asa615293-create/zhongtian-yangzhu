...
const ItemDetail: React.FC<ItemDetailProps> = ({ item, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      {/* Panel */}
      <div className="relative w-full md:w-[480px] h-full bg-bg-secondary border-l border-border-subtle slide-in-right flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-border-subtle">
          <h3 className="text-lg font-display font-semibold text-text-primary truncate pr-4">
            {item.name}
          </h3>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-text-muted hover:text-text-primary hover:bg-bg-card transition-colors flex-shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          ...
          <section>
            ...
            <FormField
              ...
              placeholder="选择空间"
            />
            {/* Show room name as display hint */}
            {item.roomId && (
              <p className="text-xs text-text-muted -mt-2 mb-4">
                当前空间：{rooms.find((r) => r.id === item.roomId)?.name || ''}
              </p>
            )}
          </section>
          ...
          {showDeleteConfirm ? (
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="px-4 py-1.5 rounded-lg text-sm font-medium bg-error/15 text-error hover:bg-error/25 transition-colors"
              >
                确认删除
              </button>
              <button
